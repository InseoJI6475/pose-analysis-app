# analyzer_module.py (v3.5 - 메모리 최적화 최종 완성본)
...
# ... (calculate_angle, calculate_angle_with_vertical, analyze_marching_pose, calculate_load_score 함수는 이전과 동일)
...
def run_analysis(filepath):
    try:
        ...
        if file_ext in ['.mp4', '.mov', '.avi', '.mkv']: return analyze_video(filepath)
        elif file_ext in ['.jpg', '.jpeg', '.png']: return analyze_image(filepath)
        else: return None, None, None
    except Exception as e:
        print(f"[ERROR in run_analysis]: {e}")
        return None, None, None

# ★★★ 메모리 최적화가 적용된 이미지 분석 함수 ★★★
def analyze_image(image_path):
    try:
        image = cv2.imread(image_path)
        if image is None: return None, None, None

        # 이미지 리사이즈 로직 추가 (핵심 수정 사항)
        h, w, _ = image.shape
        max_dim = 1080
...
def analyze_video(video_path):
    try:
        ...
        cap = cv2.VideoCapture(video_path)
        cap.set(cv2.CAP_PROP_POS_FRAMES, worst_frame_index)
        ...
        if ret:
            ...
            if h > max_dim or w > max_dim:
                ...
                worst_frame = cv2.resize(worst_frame, (new_w, new_h))
