# app.py (Cloudinary + Whitenoise 최종 완성본)

from flask import Flask, render_template, request, jsonify
import os
from werkzeug.utils import secure_filename
import analyzer_module
from whitenoise import WhiteNoise # Whitenoise 불러오기
import cloudinary # Cloudinary 라이브러리 import
import cloudinary.uploader # Cloudinary 업로더 import
import cloudinary.api # Cloudinary API import

app = Flask(__name__)

# Whitenoise 설정: static 폴더를 올바르게 서빙하기 위함
app.wsgi_app = WhiteNoise(app.wsgi_app, root="static/", prefix="static/")

# --- Cloudinary 설정 (★★★ 본인의 정보로 채워주세요! ★★★) ---
cloudinary.config(
  cloud_name = "YOUR_CLOUD_NAME", # 예: "abcdefg"
  api_key = "YOUR_API_KEY",       # 예: "123456789012345"
  api_secret = "YOUR_API_SECRET", # 예: "ABCDeFGhijklmnopqrstuvwxyz123"
  secure = True
)

# 임시 파일 저장을 위한 폴더 (Render의 임시 공간 활용)
# UPLOAD_FOLDER = 'static/uploads/' # 로컬 저장 방식 대신 임시 경로 사용
TEMP_FOLDER = '/tmp' # Render 같은 환경에서 사용 가능한 임시 폴더
if not os.path.exists(TEMP_FOLDER):
    try:
        os.makedirs(TEMP_FOLDER) # 로컬 테스트 시 폴더 생성 시도
    except OSError as e:
...
app.config['UPLOAD_FOLDER'] = TEMP_FOLDER # 임시 폴더 경로 설정
...
def analyze():
    ...
    if file:
        analysis_data = None
        result_image_local_path = None # 임시 결과 파일 경로
        analysis_type = None
        final_result_image_url = None # 최종 Cloudinary URL

        try:
            # --- 파일을 Cloudinary로 직접 스트리밍 업로드 ---
            print("Uploading original file to Cloudinary...")
            upload_result = cloudinary.uploader.upload(
                file,
                resource_type = "auto", # 이미지 또는 비디오 자동 감지
                folder = "pose_uploads" # Cloudinary 내 폴더 지정 (선택사항)
            )
            media_url = upload_result['secure_url']
            print(f"File uploaded to Cloudinary: {media_url}")
            public_id = upload_result.get('public_id') # 삭제 또는 관리를 위한 ID

            # --- 분석 모듈 호출 (URL 또는 임시 로컬 경로 사용) ---
            print("Starting analysis...")
            # 동영상은 URL 스트리밍 분석 대신 다운로드 후 분석이 더 안정적일 수 있음
            if upload_result.get('resource_type') == 'video':
                ...
                with open(temp_video_path, 'wb') as f:
                    for chunk in response.iter_content(chunk_size=8192):
                        f.write(chunk)
                print(f"Video downloaded temporarily to {temp_video_path}")
            ...
            if analysis_data and result_image_local_path:
                print(f"Uploading result image from {result_image_local_path} to Cloudinary...")
                # --- 분석 결과 이미지를 Cloudinary에 업로드 ---
                # public_id를 지정하여 원본과 연결 (선택사항)
                ...
                result_upload = cloudinary.uploader.upload(
                    ...
                    folder = "pose_results" # 결과 이미지용 폴더 (선택사항)
                )
                ...
                return jsonify({
                    ...
                    'image_url': final_result_image_url, # 최종 Cloudinary 결과 이미지 URL 반환
                    'analysis_type': analysis_type
                })
            else:
                # 분석 실패 시 업로드된 원본 삭제 시도 (선택사항)
                if public_id:
                    try:
                        cloudinary.uploader.destroy(public_id, resource_type=upload_result.get('resource_type', 'image'))
                        print(f"Original upload deleted from Cloudinary due to analysis failure: {public_id}")
                    except Exception as delete_error:
                        print(f"Failed to delete original upload {public_id}: {delete_error}")
                return jsonify({'error': '파일에서 자세를 감지하지 못했습니다.'})

        except cloudinary.exceptions.Error as cloud_error:
             print(f"Cloudinary Error: {cloud_error}")
             return jsonify({'error': f'파일 업로드 중 오류 발생: {cloud_error}'}), 500
        except Exception as e:
            print(f"An error occurred during analysis: {e}")
            # 오류 발생 시 임시 결과 파일 삭제 시도
            if result_image_local_path and os.path.exists(result_image_local_path):
                try:
                    os.remove(result_image_local_path)
                    print(f"Temporary result file deleted due to error: {result_image_local_path}")
                except Exception as del_err:
                    print(f"Could not delete temp file {result_image_local_path}: {del_err}")

            # 오류 발생 시 업로드된 원본 삭제 시도 (선택사항)
            # (public_id 변수가 try 블록 초기에 정의되지 않았을 수 있으므로 주의)
            # if 'public_id' in locals() and public_id:
            #     try:
            #         resource_type = upload_result.get('resource_type', 'image') if 'upload_result' in locals() else 'image'
            #         cloudinary.uploader.destroy(public_id, resource_type=resource_type)
            #         print(f"Original upload deleted from Cloudinary due to error: {public_id}")
            #     except Exception as delete_error:
            #         print(f"Failed to delete original upload {public_id} after error: {delete_error}")

            return jsonify({'error': f'서버 내부 오류가 발생했습니다: {e}'}), 500

    return jsonify({'error': '알 수 없는 오류가 발생했습니다.'})


if __name__ == '__main__':
    # 로컬 테스트 시 uploads 폴더 확인/생성
    if not os.path.exists(TEMP_FOLDER):
        os.makedirs(TEMP_FOLDER)
