...
EXERCISE_PRESCRIPTION_DB = {
    "back": {
        "흉·요추 후만 변형": [
            {"type": "이완", "name": "흉추 신전 운동 (Cat-Cow Stretch)", "desc": "네발기기 자세에서 등과 허리를 위아래로 움직이며 뻣뻣한 등 상부의 가동성을 회복합니다."},
            {"type": "강화", "name": "코어 안정화 운동 (Bird-dog & Dead-bug)", "desc": "팔다리를 움직이는 동안에도 척추의 중립을 유지하는 훈련을 통해 심부 안정성을 강화합니다."}
        ],
        "골반 측방 붕괴": [
            {"type": "강화", "name": "중둔근 강화 운동 (Clamshell & Side Leg Raise)", "desc": "옆으로 누워 다리를 벌리거나 들어 올리며, 골반의 좌우 안정성을 담당하는 핵심 근육을 집중적으로 강화합니다."},
            {"type": "강화", "name": "기능적 강화 운동 (Monster Walk & Single Leg Balance)", "desc": "밴드를 이용해 옆으로 걷거나 한 발로 서서, 실제 보행과 유사한 환경에서 고관절 안정성을 훈련합니다."}
        ],
        "요추 과전만 (Sway-back)": [
            {"type": "이완", "name": "고관절 굴곡근 스트레칭 (Hip Flexor Stretch)", "desc": "런지 자세에서 골반을 앞으로 밀어, 과도하게 긴장된 허리 앞쪽 근육과 허벅지 앞쪽을 이완시킵니다."},
            {"type": "강화", "name": "둔근 및 복근 강화 (Glute Bridge & Posterior Pelvic Tilt)", "desc": "누워서 엉덩이를 들거나 골반을 말아 올리며, 약화된 엉덩이/복근을 강화하여 허리의 과도한 커브를 줄여줍니다."}
        ]
    },
    "neck": {
        "거북목 증후군": [
            {"type": "강화", "name": "목 심부 굴곡근 강화 (Chin Tuck & Neck Retraction)", "desc": "턱을 당겨 뒤통수를 벽에 붙이는 동작을 통해, 목의 안정성을 담당하는 가장 중요한 심부 근육을 강화합니다."},
            {"type": "이완", "name": "후두하근 및 상부승모근 이완", "desc": "마사지볼로 목 뒤쪽을 풀어주거나, 손으로 머리를 당겨 목 옆쪽을 스트레칭하여 과긴장된 근육을 이완시킵니다."}
        ],
        "굽은 등 보상 패턴": [
            {"type": "이완", "name": "가슴 스트레칭 (Pectoral Stretch in Doorway)", "desc": "문틀을 잡고 몸을 앞으로 내밀어, 짧아지고 굽은 어깨와 가슴 근육을 활짝 펴줍니다."},
            {"type": "강화", "name": "견갑골 안정화 운동 (Wall Angels & Scapular Push-ups)", "desc": "벽에 등을 대고 팔을 움직이거나 네발기기 자세에서 날개뼈를 움직여, 등 뒤쪽의 약화된 근육을 강화하고 날개뼈의 정상적인 움직임을 회복합니다."}
        ]
    },
    "knee": {
        "대퇴사두근 우세 패턴": [
            {"type": "강화", "name": "둔근 활성화 및 강화 운동 (Glute Bridge & Single Leg RDL)", "desc": "엉덩이 근육을 먼저 사용하는 법을 다시 인지시키고 강화하여, 무릎의 부담을 근본적으로 줄여줍니다."},
            {"type": "이완", "name": "대퇴사두근 폼롤러 마사지 (Quad Foam Rolling)", "desc": "폼롤러를 이용해 과도하게 긴장된 허벅지 앞쪽 근육을 이완시켜 슬개골의 압박을 줄여줍니다."}
        ],
        "충격 흡수 부전": [
            {"type": "이완", "name": "발목 가동성 운동 (Ankle Mobility Drill)", "desc": "발목의 배측굴곡(Dorsiflexion) 가동범위를 늘려, 1차 충격 흡수 장치인 발목의 유연성을 개선합니다."},
            {"type": "강화", "name": "신경근 조절 훈련 (Low-level Plyometrics)", "desc": "가벼운 줄넘기나 제자리 점프 훈련을 통해, 충격을 부드럽게 흡수하는 신경-근육 협응 능력을 향상시킵니다."}
        ],
        "무릎 과신전 (Genu Recurvatum)": [
            {"type": "강화", "name": "햄스트링 강화 운동 (Hamstring Curls & Nordic Hamstring)", "desc": "무릎이 뒤로 꺾이지 않도록 브레이크 역할을 하는 허벅지 뒤 근육을 집중적으로 강화합니다."},
            {"type": "인지", "name": "고유수용성감각 훈련 (Proprioception Training)", "desc": "한 발로 서서 균형을 잡거나 눈을 감고 걸으며, 무릎의 정상적인 위치를 뇌가 다시 인지하도록 훈련합니다."}
        ]
    }
}
@app.route('/')
...
def analyze():
    if 'file' not in request.files: return jsonify({'error': '파일이 전송되지 않았습니다.'})
    file = request.files['file']
    if file.filename == '': return jsonify({'error': '파일을 선택해주세요.'})
    ...
    if file:
        ...
        # ★★★ 여기가 핵심 수정 사항 ★★★
        # 이제 3개의 값을 받도록 수정합니다.
        analysis_data, result_image_url, analysis_type = analyzer_module.run_analysis(filepath)
        ...
        if analysis_data:
            ...
            return jsonify({
                'basic_result': analysis_data, 'expert_diagnoses': expert_diagnoses,
                'exercise_prescriptions': exercise_prescriptions, 'image_url': result_image_url,
                'analysis_type': analysis_type
            })
        else:
            return jsonify({'error': '파일에서 자세를 감지하지 못했습니다. 터미널의 [ERROR] 메시지를 확인해주세요.'})
